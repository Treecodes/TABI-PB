...
void Clusters::compute_all_interp_pts()
{
    ...
    for (std::size_t node_idx = 0; node_idx < tree_.num_nodes(); ++node_idx) {
        ...
        for (std::size_t i = 0; i < num_interp_pts_per_node_; ++i) {
            ...
            interp_x_[node_start + i] = node_bounds[0] + (tt + 1.) / 2. * (node_bounds[1] - node_bounds[0]);
            interp_y_[node_start + i] = node_bounds[2] + (tt + 1.) / 2. * (node_bounds[3] - node_bounds[2]);
            interp_z_[node_start + i] = node_bounds[4] + (tt + 1.) / 2. * (node_bounds[5] - node_bounds[4]);
        }
    }
}
